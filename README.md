...
| `opencode-omo-config/opencode-go/omo-low-budget.jsonc` | OpenCode Go-first routing, with OpenAI reserved for the GPT-native Hephaestus agent and Sisyphus pinned to Kimi K2.7 |
| `opencode-omo-config/opencode-go/omo-mid-budget.jsonc` | The default routing strategy updated to prefer Kimi K3 for compatible orchestration and visual roles, with Sisyphus pinned to Kimi K2.7 |
| `opencode-omo-config/opencode-go/omo-high-budget.jsonc` | OpenAI Pro and OpenCode Go routing with Venice models only at the end of compatible fallback chains, Sisyphus pinned to Kimi K2.7, and no Claude models |
| `opencode-omo-config/api-access/omo-low-budget.jsonc` | Venice-only routing with no OpenCode Go or OpenAI; Sisyphus uses `venice/kimi-k2-7-code` |
| `opencode-omo-config/api-access/omo-mid-budget.jsonc` | Venice + OpenAI routing with no OpenCode Go; Sisyphus uses `venice/kimi-k2-7-code` |
| `opencode-omo-config/api-access/omo-high-budget.jsonc` | Venice + OpenAI routing with no OpenCode Go; Sisyphus uses `venice/kimi-k2-7-code` |
| `opencode-omo-config/prompts/*.md` | Prompt templates referenced by the OpenCode agent definitions |

Copy `opencode.jsonc` into your OpenCode configuration location and copy one OMO profile to `~/.omo/omo.jsonc`, then adjust model/provider names for your authenticated subscriptions. The `opencode-go/` profiles assume an active OpenCode Go subscription, and the `api-access/` profiles replace OpenCode Go models with Venice equivalents. The budget files are standalone alternatives, not files that OMO loads by those names. Run `opencode models` to confirm availability and `bunx oh-my-openagent doctor --verbose` after installation to inspect effective resolution.
