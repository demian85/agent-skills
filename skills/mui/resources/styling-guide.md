...
// Using theme.containerQueries API
...
**Caveats:**
- The `@` prefix with a unitless value renders as `px`, so `@500` is equivalent to `500px`—but `@500px` is incorrect syntax.
- Container queries must share the same units to be sorted correctly:
```typescript
// ✅ Correct
padding: { '@40em': 4, '@20em': 2, '@': 0 }
// ❌ Incorrect (mixed units)
padding: { '@40em': 4, '@50': 2, '@': 0 }
```

## CSS Cascade Layers (v9+)

Enable for better integration with Tailwind CSS v4 and other styling solutions:

```typescript
// Single layer setup (Vite/SPA)
...
**Note:** Enabling `modularCssLayers` changes specificity behavior. Theme style overrides may now take precedence over default component styles where they previously did not. Test thoroughly after enabling.
